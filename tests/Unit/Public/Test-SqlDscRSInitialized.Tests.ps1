...
Describe 'Test-SqlDscRSInitialized' {
    Context 'When validating parameter sets' {
        It 'Should have the correct parameters in parameter set <ExpectedParameterSetName>' -ForEach @(
            ...
            $result = (Get-Command -Name 'Test-SqlDscRSInitialized').ParameterSets |
    ...
    Context 'When Reporting Services is initialized' {
        BeforeAll {
    ...
    Context 'When Reporting Services is not initialized' {
        BeforeAll {
            $mockCimInstance = [PSCustomObject] @{
                InstanceName = 'SSRS'
                IsInitialized = $false
