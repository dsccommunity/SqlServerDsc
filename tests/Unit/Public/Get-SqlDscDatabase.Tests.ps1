...
Describe 'Get-SqlDscDatabase' -Tag 'Public' {
    Context 'When getting all databases' {
        BeforeAll {
            ...
            $mockServerObject | Add-Member -MemberType 'ScriptProperty' -Name 'Databases' -Value {
                $databaseCollection = @(
                    (New-Object -TypeName 'Microsoft.SqlServer.Management.Smo.Database' -ArgumentList @($mockServerObject, 'master')),
                    (New-Object -TypeName 'Microsoft.SqlServer.Management.Smo.Database' -ArgumentList @($mockServerObject, 'TestDatabase'))
                )
        ...
        It 'Should call Refresh when Refresh parameter is specified' {
            ...
            $mockServerObjectWithRefresh | Add-Member -MemberType 'ScriptProperty' -Name 'Databases' -Value {
                $databaseCollection = @(
                    (New-Object -TypeName 'Microsoft.SqlServer.Management.Smo.Database' -ArgumentList @($mockServerObjectWithRefresh, 'master'))
                )
                return $databaseCollection | Add-Member -MemberType 'ScriptMethod' -Name 'Refresh' -Value {
    ...
    Context 'When getting a specific database' {
        BeforeAll {
            $mockServerObject = New-Object -TypeName 'Microsoft.SqlServer.Management.Smo.Server'
            $mockServerObject | Add-Member -MemberType 'NoteProperty' -Name 'InstanceName' -Value 'TestInstance' -Force
            $mockServerObject | Add-Member -MemberType 'ScriptProperty' -Name 'Databases' -Value {
                return @{
                    'master' = New-Object -TypeName 'Microsoft.SqlServer.Management.Smo.Database' -ArgumentList @($mockServerObject, 'master')
                    'TestDatabase' = New-Object -TypeName 'Microsoft.SqlServer.Management.Smo.Database' -ArgumentList @($mockServerObject, 'TestDatabase')
    ...
    Context 'Parameter validation' {
        It 'Should have the correct parameters in parameter set __AllParameterSets' -ForEach @(
            ...
            $result = (Get-Command -Name 'Get-SqlDscDatabase').ParameterSets |
