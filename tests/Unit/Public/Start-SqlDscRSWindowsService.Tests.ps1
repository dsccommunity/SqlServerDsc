...
Describe 'Start-SqlDscRSWindowsService' {
    Context 'When validating parameter sets' {
        It 'Should have the correct parameters in parameter set <ExpectedParameterSetName>' -ForEach @(
            ...
            $result = (Get-Command -Name 'Start-SqlDscRSWindowsService').ParameterSets |
    ...
    Context 'When starting Windows service successfully' {
        BeforeAll {
        ...
        It 'Should not return anything' {
            $result = $mockCimInstance | Start-SqlDscRSWindowsService -Confirm:$false

            $result | Should-BeFalsy
        }
    }

    Context 'When starting Windows service with Force' {
        BeforeAll {
            $mockCimInstance = [PSCustomObject] @{
                InstanceName            = 'SSRS'
                IsWindowsServiceEnabled = $false
    ...
    Context 'When using WhatIf' {
        ...
        It 'Should not call Invoke-RsCimMethod' {
            $mockCimInstance | Start-SqlDscRSWindowsService -WhatIf
    ...
    Context 'When CIM method fails' {
        ...
        It 'Should throw a terminating error' {
            { $mockCimInstance | Start-SqlDscRSWindowsService -Confirm:$false } | Should-Throw -FullyQualifiedErrorId 'SSRSWS0001,Start-SqlDscRSWindowsService'
        }
    }
}
