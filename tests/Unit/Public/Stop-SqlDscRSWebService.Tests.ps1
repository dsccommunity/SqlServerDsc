...
Describe 'Stop-SqlDscRSWebService' {
    Context 'When validating parameter sets' {
        It 'Should have the correct parameters in parameter set <ExpectedParameterSetName>' -ForEach @(
            ...
            $result = (Get-Command -Name 'Stop-SqlDscRSWebService').ParameterSets |
    ...
    Context 'When stopping web service successfully' {
        BeforeAll {
    ...
    Context 'When stopping web service with Force' {
        BeforeAll {
            $mockCimInstance = [PSCustomObject] @{
                InstanceName            = 'SSRS'
                IsWindowsServiceEnabled = $true
