...
Describe "$($script:dscResourceName)_Integration" -Tag @('Integration_SQL2016', 'Integration_SQL2017', 'Integration_SQL2019', 'Integration_SQL2022') {
    ...
    Context ('When using configuration <_>') -ForEach @(
        "$($script:dscResourceName)_RunSqlScriptAsWindowsUser_Config"
    ) {
        BeforeAll {
            $configurationName = $_
        }
    ...
    Context ('When using configuration <_>') -ForEach @(
        "$($script:dscResourceName)_RunSqlScriptAsSqlUser_Config"
    ) {
        BeforeAll {
            $configurationName = $_
        }
    ...
    Context ('When using configuration <_>') -ForEach @(
        "$($script:dscResourceName)_RunSqlScriptWithVariablesDisabled_Config"
    ) {
        BeforeAll {
            $configurationName = $_
        }
