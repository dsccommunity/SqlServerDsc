...
Describe 'Suspend-SqlDscDatabase' -Tag @('Integration_SQL2017', 'Integration_SQL2019', 'Integration_SQL2022', 'Integration_SQL2025') {
    BeforeAll {
    ...
    Context 'When using with Get-SqlDscDatabase' {
        It 'Should take the database offline using Get-SqlDscDatabase piped to Suspend-SqlDscDatabase' {
            ...
            $resultDb = $script:serverObject | Get-SqlDscDatabase -Name $script:testDatabaseName -Refresh | Suspend-SqlDscDatabase -Force -PassThru -ErrorAction 'Stop'
            $resultDb.Status.HasFlag([Microsoft.SqlServer.Management.Smo.DatabaseStatus]::Offline) | Should -BeTrue
        }
    }
}
