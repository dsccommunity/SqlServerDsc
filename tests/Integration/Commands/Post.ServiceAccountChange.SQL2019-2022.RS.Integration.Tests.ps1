...
<#
    .NOTES
        ...
        operations for SQL Server 2019 and SQL Server 2022 Reporting Services.
        After changing the service account, these commands must run in sequence
        to restore site accessibility.

        SQL Server 2019+ uses the encryption key workflow (Remove-SqlDscRSEncryptionKey
        and New-SqlDscRSEncryptionKey) which works correctly unlike SQL Server 2017.
...
#>
Describe 'Post.ServiceAccountChange.SQL2019-2022.RS' -Tag @('Integration_SQL2019_RS', 'Integration_SQL2022_RS') {
    BeforeAll {
