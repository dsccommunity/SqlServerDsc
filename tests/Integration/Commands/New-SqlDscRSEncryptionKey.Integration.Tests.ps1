...
<#
    .NOTES
        This test is marked as potentially destructive because it regenerates the
        encryption key which will invalidate all existing encrypted data.
...
#>
Describe 'New-SqlDscRSEncryptionKey' {
    Context 'When creating a new encryption key for SQL Server Reporting Services' -Tag @('Integration_SQL2017_RS') -Skip {
        BeforeAll {
    ...
    Context 'When creating a new encryption key for SQL Server Reporting Services' -Tag @('Integration_SQL2019_RS') {
        BeforeAll {
            $script:configuration = Get-SqlDscRSConfiguration -InstanceName 'SSRS' -ErrorAction 'Stop'
        }
    ...
    Context 'When creating a new encryption key for SQL Server Reporting Services' -Tag @('Integration_SQL2022_RS') {
        BeforeAll {
            $script:configuration = Get-SqlDscRSConfiguration -InstanceName 'SSRS' -ErrorAction 'Stop'
        }
    ...
    Context 'When creating a new encryption key for Power BI Report Server' -Tag @('Integration_PowerBI') {
        BeforeAll {
