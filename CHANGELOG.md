...
- SqlAgentAlert, SqlAudit, SqlDatabase, SqlDatabasePermission, SqlPermission,
  SqlRSSetup
  - Fixed `Test()` method to return `$false` (instead of throwing) when the SQL
    Server instance is not reachable or an error occurs, enabling graceful
    handling and better support for `DependsOn` scenarios.
- SqlAG, SqlAGDatabase, SqlAGListener, SqlAGReplica, SqlAgentFailsafe,
  ...
  SqlDatabaseRole, SqlDatabaseUser, SqlEndpoint, SqlEndpointPermission, SqlLogin,
  SqlMaxDop, SqlMemory, SqlProtocol, SqlProtocolTcpIp, SqlRS, SqlReplication,
  SqlRole, SqlScript, SqlScriptQuery, SqlSecureConnection, SqlServiceAccount,
  SqlSetup, SqlTraceFlag, SqlWaitForAG, SqlWindowsFirewall
  - Fixed `Test-TargetResource` to return `$false` (instead of throwing) when
    the SQL Server instance is not reachable or an error occurs during
