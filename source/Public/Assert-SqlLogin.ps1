...
function Assert-SqlLogin
{
    ...
    process
    {
        Write-Verbose -Message ($script:localizedData.AssertLogin_CheckingPrincipal -f $Principal, $ServerObject.InstanceName)

        if (-not $ServerObject.Logins[$Principal])
        {
            $errorMessage = $script:localizedData.AssertLogin_PrincipalNotFound -f $Principal, $ServerObject.InstanceName
        ...
        Write-Verbose -Message ($script:localizedData.AssertLogin_PrincipalExists -f $Principal, $ServerObject.InstanceName)
    }
}